  ...
  function createRatingEngine({ ratingInputs, ratingDisplays, onChange }) {
    ...
    function loadRatingSprite() {
      ...
      const versionedUrl = (url) =>
        RATING_SPRITE.version ? `${url}?v=${RATING_SPRITE.version}` : url;

      function attemptLoad(url, allowFallback) {
        const spriteUrl = versionedUrl(url);
        const img = new Image();
        img.onload = () => {
        ...
        img.onerror = () => {
          if (allowFallback && RATING_SPRITE.fallbackUrl) {
            attemptLoad(RATING_SPRITE.fallbackUrl, false);
            return;
          }
      ...
      attemptLoad(RATING_SPRITE.url, true);
    }
